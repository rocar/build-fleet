...
if [ -f ".sdd/${slug}/.skip-stop-tests" ]; then
  echo "build-fleet: .sdd/${slug}/.skip-stop-tests present — stop-tests gate skipped (operator override)." >&2
  exit 0
fi
...
if [ "$n" -ge 3 ]; then
  ...
  echo "build-fleet: '${failed_cmds}' failed on ${n} consecutive stop attempts for feature '${slug}' — escalated to .sdd/${slug}/ESCALATION.md and allowing the stop." >&2
  exit 0
fi

printf '%s\n' "$n" > "$counter"
echo "build-fleet: '${failed_cmds}' failed for active feature '${slug}' (stop attempt ${n} of 3 before escalation). Tail:" >&2
echo "----" >&2
printf '%s\n' "$fail_tail" >&2
echo "----" >&2
exit 2
