...
if ! tests_exist; then
  echo "build-fleet: active bug '${slug}' is CONFIRMED but no reproducing test exists under tests/. A fix cannot land without a test that was red first (run /build-fleet:reproduce)." >&2
  echo "Refused write: ${file_path}" >&2
  exit 2
fi
