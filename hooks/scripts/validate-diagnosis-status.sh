...
if [ -z "$status_line" ]; then
  echo "build-fleet: diagnosis.md missing STATUS line. The first non-blank section (within the first 30 lines) must contain 'STATUS: REPORTED|REPRODUCING|DIAGNOSED|CONFIRMED|FIXED'." >&2
  exit 2
fi
...
case "$status_value" in
  ...
  *)
    echo "build-fleet: diagnosis.md STATUS value '${status_value}' is invalid. Must be one of: REPORTED, REPRODUCING, DIAGNOSED, CONFIRMED, FIXED." >&2
    exit 2
    ;;
esac
...
if [ ${#missing[@]} -gt 0 ]; then
  echo "build-fleet: diagnosis.md is missing required section heading(s):" >&2
  for s in "${missing[@]}"; do
    echo "  - ${s}" >&2
  done
