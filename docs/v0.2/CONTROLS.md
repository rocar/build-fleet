...
- **Cycle-3 agent-team fallback.** The agent-team promotion at CYCLE 3 in `/build-fleet:review` was conditional on `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. M1's survival-vote architecture replaces it outright; v0.2 Phase 5 cleanup removes the README env-var step and the fallback branch. **Confirmed deleted at M0 — not an open issue.**
