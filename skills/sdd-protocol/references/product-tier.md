...
```
.sdd/
  _product/              # created by /build-fleet:new-product
...
small | standard | large`. `/build-fleet:new-product` seeds `PHASE: PLAN`, `CYCLE: 0`.

## Greenfield vs brownfield

`/build-fleet:new-product` works on both. On a **greenfield** repo the architect
...
promoting the ADR). `/build-fleet:new-product` writes only `.sdd/_product/`, never
...
`/build-fleet:new-feature` runs and this file exists, it is read into the classifier +
product-owner prompts as read-only context. Features inherit the **binding** stack —
...
### PLAN_REVIEW (`/build-fleet:plan-review` → `workflows/plan-review.js`)

A fork of `review.js` (fork, don't parameterize — a deliberate design decision).
Roles `[product-owner, architect, qa]` **interrogate** the product artifacts
...
- *Bare* `/build-fleet:plan-finalize` is a **dry-run**: it prints the latest
...
`/build-fleet:new-feature` — features build against the binding stack regardless of
...
On ratification, build-fleet seeds the repo's Claude memory with the ratified product
so **any** Claude Code session (not just build-fleet commands) inherits the vision +
**binding** stack. One algorithm, two callers: `/build-fleet:plan-finalize` triggers
it on the ratify-flip (best-effort), and `/build-fleet:product-memory` is the
...
`<!-- BEGIN build-fleet:product -->` … `<!-- END build-fleet:product -->` markers. It
...
always succeeds. The escape hatch is `/build-fleet:product-memory`, run once no
...
**indented 1–3 line intent** (see below). On a successful `/build-fleet:handoff`
...
for the feature across the tier boundary, so `/build-fleet:new-feature` would
...
- **It is a sketch, not a spec.** What the feature is + its scope boundary + explicit
  ...
  adversarially reviewed at `/build-fleet:new-feature` time. The intent is *inherited
...
- **Authored** by the PO at `/build-fleet:new-product` (it already conceived each
  feature when phasing it). **Inherited** at `/build-fleet:new-feature`: the
...
- **Quality floor (canonical definition — the commands defer here).** An intent is
  **usable** only if it carries at least **2 of its 3 components**: *what the feature
  ...
  seed a spec, and `/build-fleet:new-feature` STOP-and-asks instead. The single
...
shipping feature N would leave `.sdd/ACTIVE` set and `/build-fleet:new-feature`
hard-refuses while it is non-empty).

On a **full** `/build-fleet:handoff` completion (devops succeeded + the backlog flip
...
2. **Re-resolves the next unblocked feature from the LIVE backlog** — *first
   ...
   The resolver is the **single source of truth**: `/build-fleet:handoff`,
   `/build-fleet:status`, and `/build-fleet:next-feature` all call it instead of
...
**The advancement convenience — `/build-fleet:next-feature`.** Optional. It calls the
**same resolver**, pre-checks readiness (no item in flight; the next feature's intent
passes the quality floor), and emits a dispatch signal `BUILD_FLEET_NEXT_FEATURE:
{slug, phase}` — collapsing "read `/status` → type `/new-feature <slug>`" into one
gated step. It is **convenience, not policy**: resolver only (no
reorder/skip/judgement), and it **does not run `/build-fleet:new-feature` itself** —
...
- `restrict-reviewer-writes` confines **all** writes to `.sdd/<active>/` while the
  ...
  role-based). Therefore `/build-fleet:new-product`, `/build-fleet:plan-review`, and
  `/build-fleet:plan-finalize` all **refuse to run** while a feature is in those two
