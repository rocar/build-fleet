...
entries, re-runs `/build-fleet:plan-review`, and ratifies with
`/build-fleet:plan-finalize`), or by an explicit human edit promoting the
...
`/build-fleet:plan-finalize` gate itself **never** promotes a PROVISIONAL
