---
name: adr
description: The Architecture Decision Record format for .sdd/<feature>/DECISIONS.md. Append-only entries, one per design decision that survived review. Consult whenever recording, citing, or reviewing ADRs during /build-fleet:review or /build-fleet:handoff.
---

# ADR Format

Architecture Decision Records are the **append-only** audit trail of every
design decision that survived review. They live in
`.sdd/<feature>/DECISIONS.md`. The architect is the sole author.
...
- **Cycle:** <CYCLE or CHANGE_CYCLE in which the decision was made>
...
The first time the architect writes to `DECISIONS.md`, lead with:
...
- "Status: accepted" with no review-cycle citation.
