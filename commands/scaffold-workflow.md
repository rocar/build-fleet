...
# /build-fleet:scaffold-workflow

You are the **orchestrator**. This is build-fleet's **generate-then-pin** lane: for a
...
inside build-fleet's audited `.sdd/` lanes. Design note:
`docs/history/2026-06-15-layer2-scaffold-workflow.md`.

## Modes

- `/build-fleet:scaffold-workflow <name> "<task>"` — **draft**: generate + lint + review. Never pins.
- `/build-fleet:scaffold-workflow ratify <name>` — **pin** the drafted candidate (hard lint gate).
...
1. **Parse the mode.** If the first `$ARGUMENTS` token is `ratify`, this is the **ratify** path
   ...
   > `BUILD_FLEET_REFUSE: {"command":"scaffold-workflow","code":2,"reason":"bad-args"}`

2. **Require a clean slate (both modes).** Read `.sdd/ACTIVE`. If it is non-empty, refuse:
   > `BUILD_FLEET_REFUSE: {"command":"scaffold-workflow","code":2,"reason":"item-in-flight","active":"<slug>"}`
   ...
   to finish the active item or `/build-fleet:park` it first. With nothing in flight, all of that
...
D2. **Author the candidate.** Write one self-contained workflow script to
   ...
   AND build-fleet's determinism conventions:
...
D3. **Lint (deterministic gate).** Run:
   ...
   ```
   On `BUILD_FLEET_LINT_FAIL`, surface the `BUILD_FLEET_LINT_VIOLATION` lines, fix the candidate,
...
D4. **Adversarial review (interrogation, advisory).** Fan out two reviewers in parallel via the
   Task tool — `build-fleet:architect` and `build-fleet:qa` — to **interrogate** the candidate
...
D5. **Report and stop.** Emit, before prose:
   ```
   BUILD_FLEET_SCAFFOLD_DRAFT: {"name":"<name>","lint":"pass","findings":<N>,"candidate":".sdd/_generated/<name>.js"}
   ```
   ...
   `/build-fleet:scaffold-workflow ratify <name>` to pin it. **Never pin in draft mode.**
...
R1. **Pin (hard lint gate + freeze).** Run:
   ...
   - `BUILD_FLEET_WORKFLOW_PINNED` → success.
   - `BUILD_FLEET_WORKFLOW_PIN_REFUSED` (with `reason`) → report the failure; do **not** claim the
...
R2. **Report.** On success, tell the user the workflow is frozen at `.claude/workflows/<name>.js`
   ...
   build-fleet's audited `.sdd/` lanes. It is the project's artifact now; commit it with the repo.
...
A slash command cannot set a process exit code; the `BUILD_FLEET_*` stdout signal lines are the
sole machine contract. Refusals emit one `BUILD_FLEET_REFUSE:` line carrying `"code"` (`2` =
validation / precondition refused) and `"reason"` (a kebab-case slug). The pin script's own
`BUILD_FLEET_WORKFLOW_PIN_REFUSED` line is the gate's refusal signal. Dispatch on the signal lines,
