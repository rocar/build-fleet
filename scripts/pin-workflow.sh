...
#   BUILD_FLEET_WORKFLOW_PINNED:       {"name":"<name>","path":".claude/workflows/<name>.js"}
#   BUILD_FLEET_WORKFLOW_PIN_REFUSED:  {"name":"<name>","reason":"<slug>"}
...
refuse() {  # refuse <reason-slug> <message>
  ...
  printf 'BUILD_FLEET_WORKFLOW_PIN_REFUSED: {"name":"%s","reason":"%s"}\n' "$name" "$1"
  exit 2
}
...
# Restrict to the slug charset the rest of build-fleet uses (kebab-case).
case "$name" in
...
[ -f "$candidate" ] && [ -r "$candidate" ] \
  || refuse "no-candidate" "no readable candidate at ${candidate} — draft it first with /build-fleet:scaffold-workflow."
...
if ! bash "$DIR/workflow-determinism-lint.sh" "$candidate" >/dev/null 2>&1; then
  echo "build-fleet: ${candidate} failed the determinism lint — re-run the lint to see the violations:" >&2
...
printf 'BUILD_FLEET_WORKFLOW_PINNED: {"name":"%s","path":"%s"}\n' "$name" "$dest"
echo "build-fleet: pinned ${candidate} → ${dest}. Run it with /${name} (it is now a frozen, replayable project workflow)." >&2
exit 0
