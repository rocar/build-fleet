...
# mid-flight re-prioritization is always honored. Shared by /build-fleet:handoff,
# /build-fleet:status, and (M4) /build-fleet:next-feature so the resolution logic
...
awk '
  ...
  # build-fleet script strips \r; the resolver must too).
  { gsub(/\r/, "") }
