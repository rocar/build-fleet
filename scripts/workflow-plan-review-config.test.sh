...
# node. Allowed roster differs from review.js: only {product-owner, architect, qa}
...
check("roles-default", r.error === null && eq(r.roles, ["product-owner","architect","qa"]));
r = normalizeRoles(["product-owner","qa"]);
check("roles-two-ok", r.error === null && eq(r.roles, ["product-owner","qa"]));
r = normalizeRoles(["product-owner","architect","qa"]);
check("roles-three-ok", r.error === null && eq(r.roles, ["product-owner","architect","qa"]));
r = normalizeRoles(["product-owner","product-owner","qa"]);
check("roles-dedup", r.error === null && eq(r.roles, ["product-owner","qa"]));
r = normalizeRoles(["product-owner"]);
