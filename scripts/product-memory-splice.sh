...
# (audit §3.29). Replaces the model-driven Edit splice in /build-fleet:product-memory
# and /build-fleet:plan-finalize step 7b with a tested, non-clobbering script.
#
# The target is the user-owned repo-root CLAUDE.md. Only the region between the
# build-fleet markers is ever rewritten; everything outside is preserved
# byte-for-byte. Markers (matching the sdd-protocol skill + product-memory.md):
#   BEGIN — any line starting with:  <!-- BEGIN build-fleet:product
#           (prefix match — the trailing prose may differ between versions)
#   END   — the exact line:          <!-- END build-fleet:product -->
#
...
# (the values /build-fleet:product-memory's OK signal reports).
# Exit: 0 = spliced; 1 = bad usage or corrupt markers (no write). bash 3.2.
set -uo pipefail

BEGIN_MARK='<!-- BEGIN build-fleet:product -->'
BEGIN_PREFIX='<!-- BEGIN build-fleet:product'
END_MARK='<!-- END build-fleet:product -->'
