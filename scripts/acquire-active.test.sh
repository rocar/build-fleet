...
run "$p" acquire feat-a --owner "build-fleet:new-feature" --now "2026-06-11T10:00:00Z"
...
grep -q '^OWNER: build-fleet:new-feature$' "$p/.sdd/ACTIVE.lock" \
...
# ---- status: held -------------------------------------------------------------
run "$p" status
expect "status-held" 0 '"status":"held","slug":"feat-a","owner":"build-fleet:new-feature","held_since":"2026-06-11T10:00:00Z"'

# ---- double-acquire: second loses, first's metadata intact -------------------
run "$p" acquire feat-b --owner "build-fleet:triage" --now "2026-06-11T11:00:00Z"
expect "double-acquire-refused" 1 'BUILD_FLEET_ACTIVE_CONFLICT: {"requested":"feat-b","active":"feat-a","owner":"build-fleet:new-feature","held_since":"2026-06-11T10:00:00Z"}'
printf '%s' "$err" | grep -q "build-fleet:new-feature" \
...
run "$p" acquire feat-b --owner "build-fleet:triage" --now "2026-06-11T12:00:00Z"
...
expect "legacy-active-blocks-acquire" 1 'BUILD_FLEET_ACTIVE_CONFLICT: {"requested":"feat-c","active":"old-feat","owner":"unknown"'
[ ! -f "$p/.sdd/ACTIVE.lock" ] \
